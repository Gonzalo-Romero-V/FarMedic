...
import { createLote } from "./use-stock-lotes"
...
const schema = z.object({
  ...
  cantidad_inicial: z.coerce.number({ message: "Requerido" }).int().min(1, "Debe ser ≥ 1"),
  costo_unitario: z.coerce.number({ message: "Requerido" }).nonnegative("Debe ser ≥ 0"),
})
...
/**
 * Alta de lote del empleado: sucursal_id se infiere de `useAuth()`, no es un
 * campo del formulario. Crear el lote dispara automáticamente un movimiento
 * `ingreso` en el Kardex (lo hace LoteController en backend).
 */
export function LoteAltaSheet({ medicamentos, proveedores, onCreated }: Props) {
  const { user } = useAuth()
  ...
  const onSubmit = async (values: FormOutput) => {
    if (!user?.sucursal_id) {
      toast.error("Tu usuario no tiene sucursal asignada")
      return
    }
    try {
      await createLote({
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col">
        <SheetHeader>
          <SheetTitle>Recibir lote</SheetTitle>
          <SheetDescription>
            Registra la recepción de un lote. El movimiento `ingreso` se anota automáticamente
            en el Kardex de tu sucursal.
          </SheetDescription>
        </SheetHeader>
        <Form {...form}>
          <form
        ...
        <SheetFooter className="flex-row gap-2 border-t bg-muted/30 p-4">
          ...
          <Button
            ...
            className="flex-1"
          >
            {form.formState.isSubmitting ? "Creando..." : "Registrar lote"}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
